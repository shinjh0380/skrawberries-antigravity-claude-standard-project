...
### 4. Cost Assessment (TEAM-14)

- [ ] 브로드캐스트 메시지가 진행률 보고 또는 최종 통합 외 용도로 사용되는가? (금지)
- [ ] 동일 작업 재시도가 3회를 초과하는 팀원이 있는가? (사용자 승인 필요)
- [ ] 팀원이 작업 완료 후 종료되지 않고 유휴 상태로 대기 중인가? (TEAM-13 위반)
- [ ] spawn 프롬프트에 불필요한 컨텍스트가 포함되어 있는가? (최소화 필요)

**TEAM-13 해산 기준**:
- 팀원의 success criteria 전부 충족
- 남은 작업이 1개 workstream 이하, 또는 팀원 1명으로 완료 가능
- → 해당 팀원 즉시 종료 권고 (팀 전체 해산 포함)
...
- 팀 해산 권고 가능: TEAM-13 조건 충족 시 (남은 workstream 1개 이하, 또는 팀원 1명으로 완료 가능)
